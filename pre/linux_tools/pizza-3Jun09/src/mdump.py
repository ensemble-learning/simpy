...
time,box,atoms,bonds,tris = m.viz(index)   return list of viz objects
...
class mdump:
    ...
    for snap in self.snaps:
      if snap.nflag:
        ...
        for i in xrange(len(array[0])):
          array[:,i] = Numeric.take(array[:,i],ordering)
      if snap.eflag:
        array = snap.elements
        ids = array[:,0]
        ordering = Numeric.argsort(ids)
  ...
  def read_snapshot(self,f):
    try:
      ...
      if snap.eflag: snap.eselect = Numeric.zeros(n)
      ...
      if n:
        ...
        values = Numeric.zeros((n,ncol),Numeric.Float)
        start = 0
        stop = ncol
        for i in xrange(n):
  ...
  # if called with flag, then index is timestep, so convert to snapshot number
