...
! C, 2007, 111, 15360. 
! @todo: resort the code into functions
...
Open(12, file=nlist_file, status='old')
...
DO i = 1, N ! Loop over all framework atoms
   ...
   IF (MOD(i-1, nblock) == 0) THEN
       write(*,*) "Processing ", i*100/N, "%"
   ENDIF
