        ...
        if len(nl[i]) > 0:
            ...
            for j in range(len(nl[i])):
                ...
                if ai < aj:
                    ...
                    bonds_dis.append((ai, aj))
                    o.write('%d %d %.4f\n'%(ai, aj, bond_length))
    o.close()
    n_bond = len(bond_list)
    print(n_bond, "bond terms")

    if os.path.exists('gfnff_lists.json'):
        list_xtb = read_json_file('gfnff_lists.json')
        if len(list_xtb['blist']) > 0:
            print(len(list_xtb['blist']), 'read from gfnff')
            for i in list_xtb['blist']:
                ai = min(i[0], i[1]) - 1
                aj = max(i[0], i[1]) - 1
                if not (ai, aj) in bonds_dis:
                    #print(ai+1, aj+1, atoms.get_distance(ai, aj, mic=True))
                    pass
...
def to_itp(fname, atoms, nl, bl, al, dl, charges, atomtypes):
    ...
    for i in range(len(atoms)):
        o.write("%6d    %8s    1    %s"%((i+1), atomtypes[i], molname))
...
def assign_charges(atoms):
    ...
    if os.path.exists("gfnff_charges"):
        charges = np.loadtxt("gfnff_charges")
        print("%.4f"%np.sum(charges), "total charges")
    else:
        print("Warning: no charge file found")
    return charges   
...
def assign_atom_types(atoms, nl):
    ...
    #os.system('python /home/tcheng/soft/LUNAR/atom_typing.py -topo input.pdb -f general:4')
    #os.system('python /home/tcheng/soft/LUNAR/atom_typing.py -topo input.pdb -f general-pp:2')
    os.system('python /home/tcheng/soft/LUNAR/atom_typing.py -topo input.pdb -f compass > atp.log')
    #os.system('python /home/tcheng/soft/LUNAR/atom_typing.py -topo input.pdb -f OPLS-AA')
    if os.path.exists('input_typed.nta'):
        f = open('input_typed.nta', 'r')
        n = 0
        for i in f:
            tokens = i.strip().split()
            if len(tokens) > 0 and n > 1:
                atomtypes_usr.append(tokens[1])
            n += 1
        f.close()
        print(len(atomtypes_usr), len(atomtypes))
...
if len(sys.argv) > 1:
    ...
    if len(sys.argv) > 2:
        itpfile = sys.argv[2] + '.itp'

    p = Params()

    atoms = ase.io.read(fname)
    if not fname == 'input.xyz':
        ase.io.write('input.xyz', atoms)
    ase.io.write('input.pdb', atoms)
    os.system('/home/tcheng/soft/xtb/xtb-dist/bin/xtb input.xyz --gfnff --wrtopo blist -opt > run-xtb.log')
