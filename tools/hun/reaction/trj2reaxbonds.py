	...
	for i in trjfile:
		...
		if i.find("molecular_analysis_frequency:") != -1:
			...
			for ii in range(int(natoms)):
				temp = trjfile.readline().split()
				typedic[temp[0]] = temp[1]
			result.append(typedic)
		if i.find("step:") != -1:
			nstep = i.split()[-1]
			#Skip and grep useful information 
			trjfile.readline();trjfile.readline();trjfile.readline();trjfile.readline()
			trjfile.readline();trjfile.readline()
			total_energy = trjfile.readline().split()[-1]
			total_kinetic = trjfile.readline().split()[-1]
			total_potential = trjfile.readline().split()[-1]
			bond_energy = trjfile.readline().split()[-1]
			atom_energy = trjfile.readline().split()[-1]
			lone_pair_energy = trjfile.readline().split()[-1]
			valence_angle_energy = trjfile.readline().split()[-1]
			threebody_conj = trjfile.readline().split()[-1]
			hb_energy = trjfile.readline().split()[-1]
			torsion_energy = trjfile.readline().split()[-1]
			fourbody_conj = trjfile.readline().split()[-1]
			vdw_energy = trjfile.readline().split()[-1]
			elec_energy = trjfile.readline().split()[-1]
			polar_energy = trjfile.readline().split()[-1]
			#Check if we need to skip the xyz sections
			check_npairs = int(trjfile.readline().split(',')[-1])
			if check_npairs == int(natoms):
				for k in range(int(natoms)):
					trjfile.readline()
				n_bondpairs = int(trjfile.readline().split(',')[-1])
