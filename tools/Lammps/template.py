...
dump                    2 all cfg 1 dump.*.cfg id type xs ys zs vx vy vz fx fy fz
...
thermo         1
thermo_style    custom step etotal ke pe temp press vol v_eb v_ea v_elp &
                v_emol v_ev v_epen v_ecoa v_ehb v_et v_eco v_ew v_ep v_efi &
                v_eqeq cella cellb cellc cellalpha cellbeta cellgamma
thermo_modify   line multi

dump                    1 all custom 1 dump.lmp id type x y z vx vy vz
dump_modify             1 sort id

fix            201 all box/relax tri 0.0 vmax 0.001 nreset 100 
fix            901 all reax/c/bonds 1 1 1 bonds.reax
